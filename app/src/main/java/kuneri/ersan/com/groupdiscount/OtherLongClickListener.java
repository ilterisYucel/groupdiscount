...
public class OtherLongClickListener implements View.OnLongClickListener {
    Context context;
    String id;
    ObjectProduct obj = null;
    int userId;
    public OtherLongClickListener(int userId) {
        this.userId = userId;
    }
        ...
        new AlertDialog.Builder(context).setTitle("Product Record")
                .setItems(items, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int item) {
                        if (item == 0) {
                            boolean deleteSuccessful = new TableControllerCart(context).delete(Integer.parseInt(id), userId);
